        ...
        if (redis.IsConnected)
        {
            ...
            for (int i = 0; i < qword.Count; i++)
            {
                ...
                var images = new List<IEnumerable<IImageResult>>
               {
                ...
                if (redis.GetDatabase().SetLength("image_hash_jobs") == uint.MaxValue -10000)
                {
                ...
                await Console.Out.WriteLineAsync("=====================================================================");
                await Console.Out.WriteLineAsync($"Previous done: {text}, Next: {qword[i+1]}, Redis ListLen: {redis.GetDatabase().SetLength("image_hash_jobs")} / {uint.MaxValue}");
                await Console.Out.WriteLineAsync("=====================================================================");
